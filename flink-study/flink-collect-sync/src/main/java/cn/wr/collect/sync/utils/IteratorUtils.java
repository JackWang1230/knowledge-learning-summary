...
public class IteratorUtils {
    ...
    /**
     * 根据 value值对 集合 coll 进行填充或者转换操作，将 coll中的值转换成 R 类型。
     *
     * @author liangck
     *
     * @param coll
     ...
     * @return
     */
    public static <T, V, R> List<R> fill(Collection<T> coll, final V value, BiFunction<T, V, R> filler) {
        if (CollectionUtils.isEmpty(coll))
            return Collections.emptyList();

        return coll.stream().map(item -> filler.apply(item, value)).collect(Collectors.toList());
    }

    /**
     * 根据 keyExtractor 抽取的key值对集合进行去重
     * @param keyExtractor
     * @param <T>
     * @return
     */
