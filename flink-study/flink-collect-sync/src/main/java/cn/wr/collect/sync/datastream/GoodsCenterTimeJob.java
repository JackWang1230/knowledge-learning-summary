...
import cn.wr.collect.sync.datasource.GoodsCenterTimeSource;
...
@Deprecated
public class GoodsCenterTimeJob {
    private static final Logger LOGGER = LoggerFactory.getLogger(GoodsCenterTimeJob.class);

    public static void main(String[] args) {
        LOGGER.info("GoodsCenterTimeJob start ...");
        try {
            // 配置文件路径
            ...
            if (null == parameterTool) {
                LOGGER.info("GoodsCenterTimeJob parameterTool is null");
                return;
            }
            ...
            // 查询redis连锁数据作为数据源
            DataStreamSource<PgcMerchantInfoShortInit> data = env.addSource(new GoodsCenterTimeSource())
            ...
            // 侧路输出 dtp / 非dtp
            SingleOutputStreamOperator<GoodsCenterDTO> sideOut = splitData.process(new GoodsCenterSideOutFlatMap()).setParallelism(1);
            ...
            // 执行
            env.execute("[PRD][KAFKA] - goods_center(time)");
        }
        catch (Exception e) {
            LOGGER.error("GoodsCenterTimeJob Exception: {}", e);
        }
    }
}
