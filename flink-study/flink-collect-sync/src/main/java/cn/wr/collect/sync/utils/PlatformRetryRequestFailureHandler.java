...
@Slf4j
public class PlatformRetryRequestFailureHandler implements ActionRequestFailureHandler {
    private static final long serialVersionUID = -2845509439742300398L;

    public PlatformRetryRequestFailureHandler() {
    }
    ...
    public void onFailure(ActionRequest actionRequest, Throwable throwable, int restStatusCode, RequestIndexer requestIndexer) throws Throwable {
        ...
        if (ExceptionUtils.findThrowable(throwable, EsRejectedExecutionException.class).isPresent()) {
            ...
            log.error("RetryRequestFailureHandler ElasticsearchException actionRequest:{}, onFailure:{},restStatusCode:{}",
                    actionRequest.toString(), throwable,restStatusCode);
            // 修改es字段，如果文档找不到不做处理
            ...
            // 异常3: ES语法异常，丢弃数据，记录日志
                ...
                if (exp.isPresent()) {
                    ...
                    if (null != ioExp && ioExp.getMessage() != null && ioExp.getMessage().contains("max retry timeout")) {
