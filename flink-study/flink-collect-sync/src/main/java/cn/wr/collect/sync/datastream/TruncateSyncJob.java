...
import cn.wr.collect.sync.datasource.TruncateSyncSource;
...
@Deprecated
public class TruncateSyncJob {
    private static final Logger LOGGER = LoggerFactory.getLogger(TruncateSyncJob.class);

    /**
     * main run
     */
    public static void main(String[] args) {
        LOGGER.info("TruncateSyncJob start ......");
        try {
            // 获取参数配置
            ...
            if (null == parameterTool) {
                LOGGER.info("TruncateSyncJob parameterTool is null");
                return;
            }
            ...
            // 查询redis
            DataStreamSource<BasicModel<PgConcatParams>> timeSource = env.addSource(new TruncateSyncSource())
                    .setParallelism(parameterTool.getInt(STREAM_TRUNCATE_PT_SOURCE_PARALLELISM, 5));

            // 拆分数据
            SingleOutputStreamOperator<BasicModel<Model>> goodsData = timeSource.flatMap(new Params2GoodsFlatMap())
                    .setParallelism(parameterTool.getInt(STREAM_TRUNCATE_PT_FLATMAP01_PARALLELISM, 5));

            // 拆分/组装数据
            SingleOutputStreamOperator<BasicModel<ElasticO2O>> assemblyData = goodsData.flatMap(new GoodsSplitFlatMapV2())
                    .setParallelism(parameterTool.getInt(STREAM_TRUNCATE_PT_FLATMAP02_PARALLELISM, 5));
            ...
            if (parameterTool.getBoolean(SINK_ELASTIC)) {
                ...
                assemblyData.keyBy(item -> item.getData().getSkuCode())
                        ...
                        .setParallelism(parameterTool.getInt(STREAM_TRUNCATE_PT_SINK01_PARALLELISM, 5))
                        .name("sink-elastic");
            }
            else {
            ...
            if (parameterTool.getBoolean(SINK_MBS)) {
                assemblyData.addSink(new GoodsMbsSinkV2())
                        .setParallelism(parameterTool.getInt(STREAM_TRUNCATE_PT_SINK02_PARALLELISM, 5))
                        .name("sink-mbs");
            }
            ...
            if (parameterTool.getBoolean(SINK_KAFKA)) {
                assemblyData.keyBy(item -> item.getData().getRealTradeCode() + item.getData().getMerchantId()
                        ...
                        .setParallelism(parameterTool.getInt(STREAM_TRUNCATE_PT_SINK03_PARALLELISM, 5))
                        .name("sink-kafka");
            }

            env.execute("[PRD][ES] - goods_index_2.0_real_time(truncate_pt "
                    + parameterTool.get(FLINK_COLLECT_VERSION) + ")");
        }
        catch (Exception e) {
            LOGGER.error("TruncateSyncJob error:{}", e);
        }
    }
}
