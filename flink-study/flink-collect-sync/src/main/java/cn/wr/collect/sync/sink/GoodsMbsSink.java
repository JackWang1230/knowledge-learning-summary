...
@Deprecated
public class GoodsMbsSink extends RichSinkFunction<MetricItem<ElasticO2O>> {
    private static final long serialVersionUID = -5718602542274456641L;
    private static final Logger LOGGER = LoggerFactory.getLogger(GoodsMbsSink.class);
    ...
    @Override
    public void invoke(MetricItem<ElasticO2O> value, Context context) throws Exception {
        this.sendMbs(value);
    }
     ...
     * @param metric
     */
    private void sendMbs(MetricItem<ElasticO2O> metric) {
        if (!SEND_MBS) {
            return;
        }
        ElasticO2O elasticO2O = metric.getItem();
        ...
        try {
            ...
            publishReqDTO.setMessage(new GoodsReqDTO(getChangeType(metric.getOperate()),
